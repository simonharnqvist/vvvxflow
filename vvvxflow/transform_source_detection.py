...
def transform_mods(mods: list[str], table_name: str, schema: StructField, cols_to_transform: list[str], source_path: str, detection_path: str, warehouse_path: str, db_name: str, spark: SparkSession, logger: logging.Logger):
    ...
    for mod in tqdm.tqdm(mods_to_process):
        ...
        logger.info(f"Processed mod {mod}")
...
def register_db(spark: SparkSession, db_name: str, warehouse_path: str):
    db_path = Path(warehouse_path) / f"{db_name}.db"
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {db_name} LOCATION '{db_path.as_posix()}'")

    for table_dir in db_path.iterdir():
        if table_dir.is_dir():
            fmt = "delta" if (table_dir / "_delta_log").exists() else "parquet"
            spark.sql(f"""
                CREATE TABLE IF NOT EXISTS {db_name}.{table_dir.name}
                USING {fmt}
                LOCATION '{table_dir.as_posix()}'
            """)
